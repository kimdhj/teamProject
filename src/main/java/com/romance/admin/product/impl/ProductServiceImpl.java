...
public class ProductServiceImpl implements ProductService {
	...
	@Override
	public void updateProduct(@RequestParam(name="imgfile0") MultipartFile imgfile0, @RequestParam(name="imgfile1") MultipartFile imgfile1, ProductVO vo) throws IOException {
		if (!imgfile0.isEmpty()) {
			AwsS3 awsS31 = AwsS3.getInstance();
			String dkey = null;
			if (vo.getBook_imgURL() != null) {
				dkey = vo.getBook_imgURL();
				dkey = dkey.replace("https://doublejo.s3.ap-northeast-2.amazonaws.com/", "");
			}
			if (dkey != null) {
				awsS31.delete(dkey);
			}
		...
		if (!imgfile1.isEmpty()) {
			AwsS3 awsS32 = AwsS3.getInstance();
			String dkey = null;
			if (vo.getBook_bigimgURL() != null) {
				dkey = vo.getBook_bigimgURL();
				dkey = dkey.replace("https://doublejo.s3.ap-northeast-2.amazonaws.com/", "");
			}
			if (dkey != null) {
				awsS32.delete(dkey);
