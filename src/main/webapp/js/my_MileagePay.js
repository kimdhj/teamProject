...
//구독 가격 변화 시작
$("#howmuchmileage").on("propertychange change paste input", function () {
  let abbbbb = $("#howmuchmileage").val();
  $("#myMileage_pay").number(abbbbb);
  doSumsum();
  doAddPoint();
});
...
// 합계 구합시다 시작
function doSumsum() {
  let itsSum = parseInt($("#myMileage_pay").text().replace(/,/gi, ""));
...
function doAddPoint() {
  let iitsSum =
    parseInt($("#myMileage_have").text().replace(/,/gi, "")) +
    parseInt($("#myMileage_pay").text().replace(/,/gi, ""));
  $("#myMileage_sum").number(iitsSum);
}
