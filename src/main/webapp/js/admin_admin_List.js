...
function adminAccountCheck() { // 유효성 검사
	...
	// 아이디
	const reg_id = /^[A-Za-z]{1}[A-Za-z0-9_-]{3,19}$/; // 반드시 영문시작 숫자_언더바/하이픈
														// 허용 4~20자리

	// 비밀번호
	const reg_password = /(?=.*[a-zA-ZS])(?=.*?[#?!@$%^&*-]).{8,24}/; // 문자,
																		// 특수문자
																		// 조합의
																		// 8~24자리
	...
	// 주민번호
	// const reg_registrationNum =
	// /([0-9]{2}(0[1-9]|1[0-2])(0[1-9]|[1,2][0-9]|3[0,1]))/; //주민번호
		...
		if (!reg_name_kor.test(user_name.value)
				&& !reg_name_eng.test(user_name.value)) {
		...
		if (!reg_mobile.test(user_phone.value)
				&& !reg_tel.test(user_phone.value)) {
